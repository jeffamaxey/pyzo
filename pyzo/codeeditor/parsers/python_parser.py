...
# Instance sets
python2Instance = {"self"}

python3Instance = {"self"}
...
class PythonParser(Parser):
    ...
    def _findEndOfString(self, line, token):
        ...
        style = token._style
        if endMatch := endProgs[style].search(line[token.end :]):
    ...
    def _findNextToken(self, line, pos):
        ...
        if match.group(1) is not None:
            ...
            if identifier in self._keywords:
                ...
                identifier in self._builtins
                and f".{identifier}" not in line
                and f"def {identifier}" not in line
            ):
            ...
            elif identifierState == 3 and line[match.end() :].lstrip().startswith(
                    "("
                ):
